...
import { initiateRazorpayPayment, TICKETS, type TicketId } from "@/lib/razorpay";
...
export function RegisterDialog() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await initiateRazorpayPayment({ ...parsed.data, ticket });
      setOpen(false);
    } finally {
      setSubmitting(false);
    }
  ...
  const amount = (t.amountPaise / 100).toLocaleString("en-IN");
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-md rounded-2xl">
        ...
        <form onSubmit={onSubmit} className="mt-4 space-y-3" noValidate>
          ...
          >
            {submitting ? "Opening Razorpay…" : `Pay ₹${amount} with Razorpay →`}
          </button>
          <p className="text-center text-[11px] text-muted-foreground">
            Secure payment via Razorpay. Seats are non-refundable.
          </p>
        </form>
      </DialogContent>
    </Dialog>
